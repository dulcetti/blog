...
Falae galera. Estou aqui de novo, mas agora pra mostrar um outro Tutorial/Artigo, de como fazer um menu com abas. Eu [já tinha postado um tutorial]/artigo-menu-em-abas.html), mas era utilizando imagens e ficava um pouco limitado, pois se aumentasse a fonte, a imagem naum iria ficar legal.
...
Primeiramente eu escolhi um [DOCTYPE](http://en.wikipedia.org/wiki/Doctype). Escolha o que vocês quiserem, mas eu utilizei o [XHTML](http://pt.wikipedia.org/wiki/EXtensible_Hypertext_Markup_Language) 1.1, mas fiquem livres pelo que gostarem mais.
...
[Veremos primeiro a página, sem estilos nenhum](http://www.brunodulcetti.com/artigos/menu_abas_sem/index_sem.html).

Vamos entender cada parte do codigo: `<div id="geral">` Nessa linha, teremos o div pai, que englobará todo o layout, pois fiz com um tamanho de largura fixa para seu conteúdo. Desta maneira, fica mais fácil para fazermos modificações, principalmente em posicionamento do layout.

`<div id="menu">` Div que conterá o menu, que faremos com uma lista não ordenada, <ul>.

`<li class="primeira"><a href="#" class="selecionado">Principal</a></li> <li><a href="#">Empresa</a></li> <li><a href="#">Serviços</a></li> <li><a href="#">Portfólio</a></li> <li><a href="#">Contato</a></li>`

Nessa parte, está englobada toda a codificação do menu feito em lista e seus respectivos links. Na primeira linha, temos uma classe para o link, a _selecionado_, que serve para indicar em qual página estamos. Perceba também a classe _primeira_, dentro do primeiro <li>, que no css eu explicarei para que serve.

`<div id="conteudo"> <h1>Título H1</h1> <p>Lorem ipsum dolor sit amet, consectetuer ...</p> <h2>Título H2</h2> <p>Etiam laoreet pulvinar sem. Sed massa enim ...</p> <h3>Título H3</h3> <p>Quisque convallis fringilla tortor. Duis tortor diam ...</p> </div> <div id="rodape">Copyright &copy; 2006 - <a href="http://www.brunodulcetti.com/blog/">BrunoDulcetti.com</a></div>`
...
Logicamente. Esse primeiro passo, foi somente para mostrar o código [HTML](http://pt.wikipedia.org/wiki/HTML) e só irmos jogando o css aos poucos, para vermos o resultado final.

O css completo, [você pode verificar aqui](http://www.brunodulcetti.com/artigos/menu_abas_sem/estilo.phps). Bem pequeno não é verdade?
...
`ul { list-style: none; } p { margin-bottom: 10px; } h1 { margin-bottom: 5px; color: #69A90A; font-size: 170%; } h2 { margin-bottom: 2px; color: #81C281; } h3 { color: #709AFE; }`

Aqui temos a estruturação e propriedades de todas as tags usadas no [HTML](http://pt.wikipedia.org/wiki/HTML) e que eu quis modificar. Nos meus arquivos css eu estruturo desta forma, pois fica de uma maneira mais organizada, correta, etc.
...
`#geral { margin: 10px auto; width: 750px; text-align: left; }`
...
`#menu ul { display: table; height: 1%; } #menu ul li { float: left; margin: 0 4px; } #menu ul li.primeira { margin-left: 0; } #menu ul li a { display: block; float: left; padding: 5px 12px; background-color: #CCCCCC; color: #FFFFFF; font-weight: bold; }  
...
Lembrando que aqui eu estarei listando as propriedade dentro do seletor _#menu ul li a { }_.
...
`#rodape { text-align: center; padding: 10px; } #rodape a { color: #54A4FC; } #rodape a:hover { text-decoration: none; }`
...
Bom, galera, é isso, [o resultado final pode ser conferido aqui](http://www.brunodulcetti.com/artigos/menu_abas_sem/). Qualquer dúvida, correção, melhora, crítica, elogio, é só comentar ae...
