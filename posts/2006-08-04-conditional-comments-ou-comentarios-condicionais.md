...
category: CSS
date: 2006-08-04T18:00:26.000Z
description: ''
image: null
slug: conditional-comments-ou-comentarios-condicionais
...
Ela já esta pronta, só falta eu modificar algumas coisas no Banco de Dados e adicionar o tema aqui no <a href="http://www.wordpress.org">Word Press</a>.

Falando em <a href="http://www.wordpress.org">Word Press</a>, atualizei ontem para o Word Press 2.0. E digo, ele é bem melhor que o 1.5, vários recursos interessantes, o <a href="/tchau-tchau-spammers-by-akismet.html">Akismet</a> jah vem instalado bastanto somente ativá-lo, entre outras vantagens.
...
### &raquo; Este serve como como um "Se for IE faça":

```html
<!--[if IE]> <![endif]-->
```

### &raquo; Este serve como como um "Se for superior ao IE 5 faça":

```html
<!--[if gt IE 5]> <![endif]-->
```

### &raquo; Este serve como como um "Se for IE 5 ou superior faça":

```html
<!--[if gte IE 5]> <![endif]-->
```

### &raquo; Este serve como como um "Se for anterior ao IE 5.5 faça":

```html
<!--[if lt IE 5.5]> <![endif]-->
```

### &raquo; Este serve como como um "Se for IE 6 ou anterior faça":
...
Para os que não conhecem (o que eu acho muito difícil), esseéo modo de se fazer comentário dentro do HTML. Mas a Microsoft implementou essa funcionalidade em seu browser, o i.e.

## Legal Bruno, mas continuo sem entender direito =/

Relaxa, vamos ver agora isso no HTML para que você entenda melhor a utilização. Veja abaixo o código HTML ou então <a href="/condicionais/condicional01.html">verifique o exemplo online</a>:
...
Sim, sim, mas pode ficar calmo. Foi só um exemplo para melhor compreensão. Vocês podem colocar a tag `<link>` ou a `<style>` dentro da condição, que funcionará da mesma forma. Veja abaixo o exemplo ou <a href="/condicionais/condicional02.html">confira online</a>:
...
Neste exemplo, utilizei a tag `<link>` dentro do `<head>` e dentro da condição. Dentro do `<head>` eu chamei o arquivo <a href="/condicionais/normal.css">normal.css</a>, que serve para todos os browsers e temos tb o arquivo <a href="/condicionais/ie.css">ie.css</a> que serve somente para o i.e. e está dentro da condição.
